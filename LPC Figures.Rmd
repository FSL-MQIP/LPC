...
```{r Descriptive stats for LPC isolates, echo=FALSE}
LPC_Concentration <- Isolates_Query %>%
  distinct(ID, Average_Concentration) %>%
  mutate(log_concentration = log10(Average_Concentration))

stat.desc(LPC_Concentration$log_concentration)
...
```{r histogram for LPC distribution, echo=FALSE}
LPC_Distribution_histogram <- ggplot(LPC_Concentration, aes(x = log_concentration)) + 
    geom_histogram(aes(y =..density..),
                   breaks = seq(0, 5, by = 0.2), 
                   colour = "black", 
                   fill = "white") + xlab("LPC (log CFU/mL)") + ggtitle("LPC Distribution") +
stat_function(fun = dnorm, args = list(mean = mean(LPC_Concentration$log_concentration), sd = sd(LPC_Concentration$log_concentration)))
